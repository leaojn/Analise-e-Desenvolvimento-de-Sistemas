...
from tep.models import Post, Comment
...
for item in json["users"]:
    ...
    user = User()
    user.username = nome
    user.set_password(nome)
    user.email = email
    user.save()
    Perfil.objects.create(name=nome, phone=phone, email=email, city=city,
                        street=street, suite=suite, zipcode=zipcode, user=user)
    print("User criado")
for i in json['posts']:
    title = i['title']
    body = i['body']
    userid = i['userId']
    Post.objects.create(title=title, body=body, perfil_id=userid, owner_id=userid)
    print("Post criado")
...
for i in  json['comments']:
  ...
  email = i['email']
  body = i['email']
  Comment.objects.create(post_id=post, name=name, email=email, body=body)
